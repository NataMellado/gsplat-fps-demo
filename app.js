...
function animate() {
  ...
  if (Math.abs(lookJoystick.x) > 0.05 || Math.abs(lookJoystick.y) > 0.05) {
    ...
    euler.y -= lookJoystick.x * sensibilidad * delta;
    euler.x += lookJoystick.y * sensibilidad * delta;
    const limite = 0.4;
    euler.x = Math.max(-limite, Math.min(limite, euler.x));
    camera.quaternion.setFromEuler(euler);
  }
  ...
  const LIMITS = {
    xMin: -0.9, // Pared izquierda
    xMax: 0.93, // Pared derecha
    zMin: -0.7888669527503072, // Pared frente
    zMax: 0.7888669527503072, // Pared fondo
  };
  camera.position.x = Math.max(
    LIMITS.xMin,
    Math.min(LIMITS.xMax, camera.position.x),
  );
  camera.position.z = Math.max(
    LIMITS.zMin,
    Math.min(LIMITS.zMax, camera.position.z),
  );
